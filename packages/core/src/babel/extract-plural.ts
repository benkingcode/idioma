...
/**
 * Serialize a Select JSX element to ICU MessageFormat.
 *
 * Converts:
 ...
 */
export function serializeSelectToIcu(element: t.JSXElement): SelectIcuResult {
  const opening = element.openingElement;
  ...
  for (const attr of opening.attributes) {
    ...
    if (name === 'value') {
      ...
      // Any other prop is a selection option (except React reserved props)
      if (t.isStringLiteral(attr.value)) {
        props.set(name, attr.value.value);
      } else if (
        t.isJSXExpressionContainer(attr.value) &&
  ...
  // Validate required props
  if (!variable) {
    throw new Error('Select requires a value prop');
  }
...
/**
 * Serialize a SelectOrdinal JSX element to ICU MessageFormat.
 *
 * Converts:
 ...
 */
export function serializeSelectOrdinalToIcu(
  ...
  // Validate required props
  if (!variable) {
    throw new Error('SelectOrdinal requires a value prop');
  }
