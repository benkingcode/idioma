 ...
 */
export async function extractMessages(options: ExtractOptions): Promise<ExtractResult> {
  const { cwd, sourcePatterns, localeDir, defaultLocale, locales, clean } = options
...
function messagesToCatalog(messages: ExtractedMessage[], locale: string): Catalog {
...
export const extractCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    console.log(`Extracted ${result.messages.length} messages from ${result.files} files`)
